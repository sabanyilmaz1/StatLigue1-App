...
interface JoueurScreenProps extends NavigationProps {
  route: RouteProp<RootStackParamList, 'Joueur'>
}
...
> {
  ...
  componentDidMount() {
    //On recupere l'identifiant du joueur cliqué à la vue JoueurSearchScreen
    const idJoueur = this.props.route.params.joueurId
    // On peut recuperer les données sur le joueur cliqué à l'aide de son identifiant
    joueurApiService.getPlayersById(idJoueur).then((joueur: Joueur) => {
      // Changement du nom de la vue avec le prenom et nom du joueur
  ...
  componentDidUpdate() {
    //On recupere la photo du joueur via une autre api
